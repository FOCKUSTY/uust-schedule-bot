...
export class WebsiteScheduleProvider implements ScheduleProvider {
  ...
  public async getFullSchedule(group: GroupInformation): Promise<ScheduleWeeks> {
    ...
    throw new Error("getFullSchedule не поддерживается для WebsiteScheduleProvider");
  }

  public async getWeekSchedule(group: GroupInformation, weekNumber: number): Promise<ScheduleWeek> {
    const urls = await this.loadUrls();
    const baseUrl = urls[group.group];
    if (!baseUrl) {
      throw new Error(`URL для группы ${group.group} не найден`);
  ...
  private convertToScheduleWeek(rows: string[][], weekNumber: number): ScheduleWeek {
