...
import { REGISTRATION_CONVERSATION, registrationConversation } from './conversations/registration';
import { SCHEDULE_CONVERSATION, scheduleConversation } from "./conversations/schedule";
...
bot.use(createConversation<Context, Context>((conversation, context) => {
...
bot.on('callback_query:data', async (interaction) => {
  ...
  if (data.startsWith('select_config:')) {
    ...
    try {
      ...
      return sendOrEditMessage(interaction, "Главное меню", { keyboard: mainMenuKeyboard() });
    } catch {
      ...
      return sendOrEditMessage(interaction, "Пожалуйста, выберите группу из списка:", { keyboard });
    }
  }
});
