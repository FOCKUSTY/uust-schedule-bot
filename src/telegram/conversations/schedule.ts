...
import { DAY_NAMES_RU, getDayIndexForToday, resolveDayOffset } from "../schedule";
...
export const scheduleConversation = async (
  ...
  const defaultConfig = configs.map((config, index) => ({ config, index })).filter((data) => data.config.isDefault)[0];
  
  if (configs.length === 0 || !defaultConfig) {
    return sendOrEditMessage(interaction, "Выберите группу", { keyboard: configSelectionKeyboard(configs), conversation });
  }
  ...
  if (!dayName) {
    return sendOrEditMessage(interaction, "Произошла ошибка, извините", { conversation });
  }
  ...
  const keyboard = (() => {
    ...
    if (session.watchType === "day") {
      keyboard.text("⬅️", "schedule:day:previous").text(`📅 ${dayName}`, "schedule:day:reset").text("➡️", "schedule:day:next").row();
      keyboard.text("🗓 На неделю", "schedule:switch:toweek").row();
    } else {
      keyboard.text("⬅️", "schedule:week:previous").text(`📅 Неделя ${currentWeek}`).text("➡️", "schedule:week:next").row();
      keyboard.text("🗓 На день", "schedule:switch:today").row();
    }

    keyboard.text("🔄 Сменить группу", "menu:switch_group").text(`${configs[currentGroupIndex].group}`, "schedule:week:reset").row();
